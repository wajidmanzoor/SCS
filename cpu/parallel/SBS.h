...
// Decide whether this MPI worker owns a binary-search child. Before the split
// depth, every worker follows the small common prefix. At the split depth,
// complete prefixes are assigned modulo bb_shard_count; below it, the owning
// worker explores the entire subtree. Every dom=0 state therefore belongs to
// exactly one shard, while nodes above the frontier are duplicated deliberately.
inline bool bb_shard_child(unsigned int branch_bit,
                           unsigned long long prefix,
                           ui prefix_depth,
                           unsigned long long &child_prefix,
                           ui &child_prefix_depth)
{
    if(bb_shard_count <= 1 || prefix_depth >= bb_shard_split_depth){
        child_prefix = prefix;
        child_prefix_depth = prefix_depth;
        return true;
    }

    child_prefix = (prefix << 1) | (branch_bit & 1U);
    child_prefix_depth = prefix_depth + 1;
    if(child_prefix_depth < bb_shard_split_depth) return true;
    return (child_prefix % bb_shard_count) == bb_shard_id;
}

// Description : SBS algorithm without domination branching technique.
void BB(ui level, unsigned long long shard_prefix = 0,
        ui shard_prefix_depth = 0)
{   if (verbose){
    ...
    if(ub > kl && q_dist[ustar] <= ubD){
        ...
        unsigned long long child_prefix = shard_prefix;
        ui child_prefix_depth = shard_prefix_depth;
        if(bb_shard_child(1, shard_prefix, shard_prefix_depth,
                          child_prefix, child_prefix_depth)){
            BB(level + 1, child_prefix, child_prefix_depth);
        }
    }
    ...
    if(ub > kl){
        ...
        unsigned long long child_prefix = shard_prefix;
        ui child_prefix_depth = shard_prefix_depth;
        if(bb_shard_child(0, shard_prefix, shard_prefix_depth,
                          child_prefix, child_prefix_depth)){
            BB(level + 1, child_prefix, child_prefix_depth);
        }
    }
