...
bool isServerExit(const std::string &str) {
  ...
  std::transform(trimmedStr.begin(), trimmedStr.end(), trimmedStr.begin(),
                 ::tolower);
...
void processMessages() {
  ...
  while (true) {
    ...
    while ((!messageQueue.empty()) && (numQueriesProcessing < limitQueries)) {
      ...
      queries[ind].updateQueryData(argValues[0], argValues[1], argValues[2],
                                   argValues[3], argValues[4], queryId, ind);
      if (queries[ind].isHeu)
        CSSC_heu(ind);
      if (queries[ind].kl == queries[ind].ku) {
        stringstream ss;
        ss << queries[ind].N1 << "|" << queries[ind].N2 << "|"
           << queries[ind].QID << "|"
           << integer_to_string(queries[ind].receiveTimer.elapsed()).c_str()
           << "|" << queries[ind].kl << "|" << "0" << "|" << "1|0";
      ...
      chkerr(cudaMemcpy(deviceGraph.degree + (ind * n), degree, n * sizeof(ui),
                        cudaMemcpyHostToDevice));
      chkerr(cudaMemcpy(deviceGraph.distance + (ind * n), q_dist,
                        n * sizeof(ui), cudaMemcpyHostToDevice));
      chkerr(cudaMemcpy(deviceGraph.lowerBoundDegree + ind, &(queries[ind].kl),
                        sizeof(ui), cudaMemcpyHostToDevice));
      chkerr(cudaMemcpy(deviceGraph.lowerBoundSize + ind, &(queries[ind].N1),
                        sizeof(ui), cudaMemcpyHostToDevice));
      chkerr(cudaMemcpy(deviceGraph.upperBoundSize + ind, &(queries[ind].N2),
                        sizeof(ui), cudaMemcpyHostToDevice));
      chkerr(cudaMemcpy(deviceGraph.limitDoms + ind, &(queries[ind].limitDoms),
                        sizeof(ui), cudaMemcpyHostToDevice));

      chkerr(cudaMemset(initialTask.globalCounter, 0, sizeof(ui)));
      chkerr(cudaMemset(initialTask.entries, 0, INTOTAL_WARPS * sizeof(ui)));
      chkerr(cudaMemset(deviceGraph.newOffset + ((n + 1) * ind), 0,
                        (n + 1) * sizeof(ui)));
      ...
      if (queries[ind].kl <= 1)
        ...
        for (ui d = 1; d <= queries[ind].N2; d++) {
          if (d == 1 || d == 2) {
            ...
            ui min_n =
                queries[ind].kl + d + 1 + floor(d / 3) * (queries[ind].kl - 2);
      ...
      initialReductionRules<<<BLK_NUM2, BLK_DIM2, sharedMemorySizeinitial>>>(
          deviceGenGraph, deviceGraph, initialTask, n, queries[ind].ubD,
          initialPartitionSize, ind);
      cudaDeviceSynchronize();
      CUDA_CHECK_ERROR("Intial Reduction ");

      ui globalCounter;
      chkerr(cudaMemcpy(&globalCounter, initialTask.globalCounter, sizeof(ui),
                        cudaMemcpyDeviceToHost));

      ui writeWarp, ntasks, space;
      chkerr(cudaMemcpy(&writeWarp, deviceTask.sortedIndex, sizeof(ui),
                        cudaMemcpyDeviceToHost));
      chkerr(cudaMemcpy(&ntasks, deviceTask.numTasks + writeWarp, sizeof(ui),
                        cudaMemcpyDeviceToHost));
      ui offsetPsize = partitionSize / factor;
      chkerr(cudaMemcpy(
          &space, deviceTask.taskOffset + (writeWarp * offsetPsize + ntasks),
      ...
      CompressTask<<<BLK_NUM2, BLK_DIM2>>>(
          deviceGenGraph, deviceGraph, initialTask, deviceTask,
          initialPartitionSize, queries[ind].QID, ind, n, partitionSize,
          TOTAL_WARPS, factor);
      cudaDeviceSynchronize();
      CUDA_CHECK_ERROR("Compress ");

      thrust::inclusive_scan(
          thrust::device_ptr<ui>(deviceGraph.newOffset + ((n + 1) * ind)),
          thrust::device_ptr<ui>(deviceGraph.newOffset + ((n + 1) * (ind + 1))),
          thrust::device_ptr<ui>(deviceGraph.newOffset + ((n + 1) * ind)));
      cudaDeviceSynchronize();

      numQueriesProcessing++;

      NeighborUpdate<<<BLK_NUMS, BLK_DIM, sharedMemoryUpdateNeigh>>>(
          deviceGenGraph, deviceGraph, deviceTask, TOTAL_WARPS, ind, n, m,
          partitionSize, factor);
      cudaDeviceSynchronize();
      ...
      thrust::device_vector<ui> d_temp_input(d_sortedIndex_ptr,
                                             d_sortedIndex_ptr + TOTAL_WARPS);

      thrust::sequence(thrust::device, d_sortedIndex_ptr,
                       d_sortedIndex_ptr + TOTAL_WARPS);

      thrust::sort_by_key(thrust::device, d_temp_input.begin(),
                          d_temp_input.end(), d_sortedIndex_ptr,
                          thrust::less<ui>());

      thrust::scatter(thrust::device, thrust::make_counting_iterator<ui>(0),
                      thrust::make_counting_iterator<ui>(TOTAL_WARPS),
                      d_sortedIndex_ptr, d_mapping_ptr);

      thrust::transform(thrust::device, d_mapping_ptr,
                        d_mapping_ptr + TOTAL_WARPS, d_mapping_ptr,
                        subtract_from(TOTAL_WARPS - 1));
      // ui *neighboroffset, *neighborList;
      // neighboroffset = new ui[n];
      // neighborList = new ui [2*m];

      //        chkerr(cudaMemcpy(neighboroffset,deviceGraph.newOffset + (ind *
      //        (n+1)), (n+1) * sizeof(ui), cudaMemcpyDeviceToHost));
      //      chkerr(cudaMemcpy(neighborList,deviceGraph.newNeighbors + (ind *
      //      (2*m)), (2*m) * sizeof(ui), cudaMemcpyDeviceToHost));
      //    distance_after_reduction(queries[ind].QID,n, neighboroffset,
      //    neighborList);

      //  chkerr(cudaMemcpy(deviceGraph.distance + (ind * n), q_dist, n *
      //  sizeof(ui), cudaMemcpyHostToDevice));
    ...
    if (numQueriesProcessing != 0) {
      ...
      chkerr(cudaMemset(deviceTask.doms, 0,
                        TOTAL_WARPS * partitionSize * sizeof(ui)));

      sharedMemorySizeTask =
          2 * WARPS_EACH_BLK * sizeof(ui) + WARPS_EACH_BLK * sizeof(int) +
          WARPS_EACH_BLK * sizeof(double) + 2 * WARPS_EACH_BLK * sizeof(ui) +
          maxN2 * WARPS_EACH_BLK * sizeof(ui);

      ProcessTask<<<BLK_NUMS, BLK_DIM, sharedMemorySizeTask>>>(
          deviceGenGraph, deviceGraph, deviceTask, partitionSize, factor, maxN2,
          n, m, dMAX, limitQueries, red1, red2, red3, prun1, prun2);
      cudaDeviceSynchronize();
      CUDA_CHECK_ERROR("Process Task");
      chkerr(cudaMemset(deviceTask.doms, 0,
                        TOTAL_WARPS * partitionSize * sizeof(ui)));
      // This kernel identifies vertices dominated by ustar and sorts them in
      // decreasing order of their connection score.
      FindDoms<<<BLK_NUMS, BLK_DIM, sharedMemorySizeDoms>>>(
          deviceGenGraph, deviceGraph, deviceTask, partitionSize, factor, n, m,
          dMAX, limitQueries);
      cudaDeviceSynchronize();
      CUDA_CHECK_ERROR("Find Doms");

      // This kernel writes new tasks, based on ustar and the dominating set,
      // into the task array or buffer. It reads from the buffer and writes to
      // the task array.
      // ui flag = 1 ;
      // chkerr(cudaMemcpy( &flag, &deviceBuffer.outOfMemoryFlag,
      // sizeof(ui),cudaMemcpyDeviceToHost));
      Expand<<<BLK_NUMS, BLK_DIM, sharedMemorySizeExpand>>>(
          deviceGenGraph, deviceGraph, deviceTask, deviceBuffer, partitionSize,
          factor, copyLimit, bufferSize, numTaskHost - numReadHost, readLimit,
          n, m, dMAX, limitQueries);
      cudaDeviceSynchronize();
      CUDA_CHECK_ERROR("Expand ");
      RemoveCompletedTasks<<<BLK_NUMS, BLK_DIM>>>(deviceGraph, deviceTask,
                                                  partitionSize, factor);
      cudaDeviceSynchronize();
      CUDA_CHECK_ERROR("Remove Completed ");

      chkerr(cudaMemcpy(&(outMemFlag), deviceBuffer.outOfMemoryFlag, sizeof(ui),
                        cudaMemcpyDeviceToHost));
      ...
      chkerr(cudaMemcpy(queryStopFlag, deviceGraph.flag,
                        limitQueries * sizeof(ui), cudaMemcpyDeviceToHost));

      for (ui i = 0; i < limitQueries; i++) {
        if ((queryStopFlag[i] == 0) && (queries[i].solFlag == 0)) {
          chkerr(cudaMemcpy(&(queries[i].numRead), deviceGraph.numRead + i,
                            sizeof(ui), cudaMemcpyDeviceToHost));
          chkerr(cudaMemcpy(&(queries[i].numWrite), deviceGraph.numWrite + i,
                            sizeof(ui), cudaMemcpyDeviceToHost));
          if ((queries[i].numRead == queries[i].numWrite)) {
            chkerr(cudaMemcpy(&(queries[i].kl),
                              deviceGraph.lowerBoundDegree + i, sizeof(ui),
                              cudaMemcpyDeviceToHost));

            stringstream ss;
            ss << queries[i].N1 << "|" << queries[i].N2 << "|" << queries[i].QID
               << "|"
               << integer_to_string(queries[i].receiveTimer.elapsed()).c_str()
               << "|" << queries[i].kl << "|" << "0" << "|" << "0|0";
            writeOrAppend(fileName, ss.str());
      ...
      if (numTaskHost == numReadHost) {
        ...
        chkerr(cudaMemset(deviceBuffer.taskOffset, 0,
                          (numReadHost + 1) * sizeof(ui)));
      }

      // If the number of tasks written to the buffer exceeds the number read at
      // this level, left shift the tasks that were written but not read to the
      // start of the array.
      if ((numReadHost < numTaskHost) && (numReadHost > 0)) {
        chkerr(cudaMemcpy(&startOffset, deviceBuffer.taskOffset + numReadHost,
                          sizeof(ui), cudaMemcpyDeviceToHost));
        chkerr(cudaMemcpy(&endOffset, deviceBuffer.taskOffset + numTaskHost,
                          sizeof(ui), cudaMemcpyDeviceToHost));
        ...
        chkerr(cudaMemset(deviceBuffer.taskOffset +
                              (numTaskHost - numReadHost + 1),
                          0, numReadHost * sizeof(ui)));
        ...
        thrust::copy(
            thrust::device_ptr<ui>(deviceBuffer.queryIndicator + numReadHost),
    ...
    if (c == 4000) {
      for (ui i = 0; i < limitQueries; i++) {
        if (queries[i].solFlag == 0) {
          chkerr(cudaMemcpy(&(queries[i].kl), deviceGraph.lowerBoundDegree + i,
                            sizeof(ui), cudaMemcpyDeviceToHost));
  ...
  stringstream ss;
  ss << "-1|-1|-1|-1|-1|-1|-1|"
     << integer_to_string(totalTimer.elapsed()).c_str();
  writeOrAppend(fileName, ss.str());
}
...
int main(int argc, const char *argv[]) {
  ...
  const char *filepath =
      argv[1]; // Path to the graph file. The graph should be represented as an
               // edge list with tab (\t) separators
  partitionSize =
      atoi(argv[2]); // Defines the partition size, in number of elements, that
                     // a single warp will read from and write to.
  bufferSize =
      atoi(argv[3]); // Specifies the size, in number of elements, where warps
                     // will write in case the partition overflows.
  copyLimit =
      stod(argv[4]); // Specifies that only warps with at most this percentage
                     // of their partition space filled will read from the
                     // buffer and write to their partition.
  readLimit = atoi(argv[5]); // Maximum number of tasks a warp with an empty
                             // partition can read from the buffer.
  ...
  fileName = "results/results_" + fileName;
  ...
  if (n <= WARPSIZE) {
    ...
    BLK_NUM2 =
        std::min(BLK_NUMS,
                 static_cast<int>(std::ceil(static_cast<float>(n) / BLK_DIM2)));
  }

  INTOTAL_WARPS = (BLK_NUM2 * BLK_DIM2) / WARPSIZE;

  initialPartitionSize =
      static_cast<ui>(std::ceil(static_cast<float>(n) / INTOTAL_WARPS));
