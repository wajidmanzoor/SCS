...
void heu2(vector<ui>& H2, ui& kl2,ui N1,ui N2, ui QID) {
  ...
  // Description: array. 0 means not in H or Queue, 1 means in Queue and 2 means
  // in Temporary H
  ui* sta = new ui[n];
  ...
  // Description: Priority queue of vertices (v) based on degree of vertex if
  // added in Temporary H.i.e degree_(T U v)
  ...
  while (!Q.empty()) {
    ...
    for (ui j = pstart[v]; j < pstart[v + 1]; j++) {
      ...
      // Description : If neighbor (nei) is neither in Temporary H nor in
      // priority queue.
      if (sta[nei] == 0) {
        ui d = 0;

        // Description : calculate num of neighbors of nei that are in Temporary
        // H. I.e The degree of vertex (nei) in Temporary H if added in
        // temporary H.
        ...
        // Description : if neighbor in temporary H
          ...
          // Description : Update the degree of neighbor and the vertex in
          // Tempoary H.
          ++deg[nei];
          ++deg[v];
        }
    ...
    if (tH.size() >= N1) {
      ...
      for (ui i = 0; i < tH.size(); i++) {
        if (deg[tH[i]] < mindeg) mindeg = deg[tH[i]];
      }

      // Description: if current minumn degree > max minimum degree. Update max
      // minimum degree. Issue: we should also update the H
      if (mindeg >= tkl) {
        tkl = mindeg;
        tH_size = (ui)tH.size();
      }
...
void heu3(vector<ui>& H3, ui& kl3,ui N1,ui N2, ui QID) {
  ...
  while (!Q.empty()) {
    ...
    // Description : Iterate through all the neghbors to update the degree of
    // vertecies that all already in Temporary H
    ...
    for (ui nei = pstart[v]; nei < pstart[v + 1]; nei++) {
      ...
      if (!sta[edges[nei]]) {
        ...
        // Description : Iterate through neighbors of nei that are in Temporary
        // H. To calculate the connection 1/(degree of neighbors).
...
void heu4(vector<ui>& H4, ui& kl4,ui N1,ui N2, ui QID) {
  ...
  for (auto e : S) Q.push(make_pair(deg[e], e));
  vector<ui> rmv;
  ui rmv_idx = 0;
  ui mindeg = 0;
  while (!Q.empty()) {
  ...
  memset(inS, 0, sizeof(bool) * n);
  for (auto e : S) inS[e] = 1;
  for (ui i = 0; i < rmv_idx; i++) inS[rmv[i]] = 0;
